...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  const response = await fetch(
    "https://35.154.225.94/funds/overall-portfolio?format=json&fundName=VII%20Ventures%20FUND%202%20SP"
  );
...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  const response = await fetch(
    "https://35.154.225.94/funds/company-level-portfolio?format=json&fundName=VII%20Ventures%20FUND%202%20SP"
  );
  const responseData = await response.json();
